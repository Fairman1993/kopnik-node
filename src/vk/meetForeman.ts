  ...
  await friends([foreman], {wait: true}, async () => {
    ...
    result = await meet(`10-ка ${foreman.firstName}`,
      [foreman,],
      {
        ...
        data: {
          message: join([
            `$t ${link(foreman, LinkMode.i)}! Настал новый этап в твоей жизни, теперь ты старшина!`,
            `Быть старшиной значит, что другие копные мужи возложили на тебя ответственность представлять их семьи на всех копах, на  которые тебя пригласят и на которых ты будешь участвовать как старшина общины. Относись к своим новым обязанностям ответственно и добросовестно! Помни, что старшина это временная должность: тот кто тебя выбрал, волен тебя и снять.`,
