...
export default async function (halfUser: User, witness: User, neighbors: User[], prevStatus: StatusEnum, changesetTranslated: string[]): Promise<Chat> {
  let result: Chat
    ...
    [halfUser, witness, testUser()],
    {
      ...
      data: {
        message: t(messageKey, {
          halfUser: link(halfUser, LinkMode.i),
          neighbors: neighbors.map(eachNeighbor=>link(eachNeighbor, LinkMode.i)).join(', '),
          witness: link(witness, LinkMode.i),
          changeset: list(changesetTranslated)
        })
      }
    })
